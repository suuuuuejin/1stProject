  			...
  			success : function(data){
  				...
  				$.each(data,function(intdex,obj){
  					...
  					if(obj.login_id=="${mvo.login_id}"){
						...
						comm+='<div class="col">';
						comm+='<span id="cmtbtn'+obj.zw_cmt_seq+'"><button type="button" class="btn btn-success w-100" onclick="commUpdateForm('+obj.zw_cmt_seq+')">수정</button></span>';
				...
				<div class="container-xl">
					...
					<div class="page-header d-print-none">
						<div class="row g-2 align-items-center">
							<div class="col">
								<h2 class="page-title">
									<a href="${cpath }/zwlist.do">제로웨이스트/비건</a>
								</h2>
							</div>
						</div>
					</div>
						...
						<div class="row row-cards">
							<table class="table mb-0" id="view">
								<tr>
									...
									<Td><c:if test="${vo.notice eq '' }">
											<span class="text-green">[${vo.zw_headline}]<svg
													...
													<path
														d="M9 18c6.218 0 10.5 -3.288 11 -12v-2h-4.014c-9 0 -11.986 4 -12 9c0 1 0 3 2 5h3z"></path></svg></span>
										</c:if> ${vo.zw_title }</Td>
								...
								<tr>
									...
									<td colspan="3"><c:if test="${vo.zw_file != ' ' }">
											<img src="${cpath}/upload/${vo.zw_file}">
											<Br>
										</c:if> ${fn:replace(vo.zw_content,newLineChar,"<br/>")}</td>
												...
												<c:choose>
													<c:when test="${vo.notice eq 0 }">
														<button class="btn  btn-warning"
															onclick="goNotice(${vo.zw_seq})">공지</button>
													</c:when>
													<c:otherwise>
														<button class="btn btn-warning"
															onclick="delNotice(${vo.zw_seq})">공지 제거</button>
													</c:otherwise>
												</c:choose>
												<button class="btn  btn-success"
													onclick="goUpdate(${vo.zw_seq})">수정</button>
												<button class="btn  btn-danger"
													onclick="goDel(${vo.zw_seq})">삭제</button>
											</c:when>
													...
													<div class="card-body">
														<form id="writeComment">
															<div class="row">
																...
																<div class="col-7">
																	<input type="hidden" name="zw_seq"
																		value="${vo.zw_seq }"> <input type="hidden"
																		name="login_id" value="${mvo.login_id }">
																	<textarea class="form-control mt-3" id="content"
																		type="text" name="zw_cmt_content" rows="3"

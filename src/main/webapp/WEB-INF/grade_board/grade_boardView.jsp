  			...
  			success : function(data){
  				...
  				$.each(data,function(intdex,obj){
					...
					if(obj.login_id=="${mvo.login_id}"){
						...
						comm+='<div class="col">';
						comm+='<span id="cmtbtn'+obj.grade_cmt_seq+'"><button type="button" class="btn btn-success w-100" onclick="commUpdateForm('+obj.grade_cmt_seq+')">수정</button></span>';
	    			...
	    			<c:when test="${mvo.u_type eq 'admin' }">
	    				<c:choose>
			    			<c:when test="${vo.notice eq 0 }">
			    				...
			    				<button class="btn btn  btn-warning" onclick="goNotice(${vo.grade_seq})">공지</button>
			    			</c:when>
			    			<c:otherwise>
			    				<button class="btn btn  btn-warning" onclick="delNotice(${vo.grade_seq})">공지 제거</button>
			    			</c:otherwise>
	    				</c:choose>
	    				<button class="btn btn  btn-success" onclick="goUpdate(${vo.grade_seq})">수정</button>
	    				<button class="btn btn  btn-danger" onclick="goDel(${vo.grade_seq})">삭제</button>
	    			</c:when>
